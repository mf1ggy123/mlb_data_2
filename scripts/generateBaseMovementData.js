...
function parseCSV() {
  const csvPath = path.join(__dirname, '../base_movement_values.csv');
...
function generateTSFile() {
  ...
  let tsContent = `// Auto-generated base movement values from CSV
...
`;
  ...
  const outputPath = path.join(__dirname, '../utils/baseMovementValues.ts');
  fs.writeFileSync(outputPath, tsContent);
  console.log(`Generated ${Object.keys(data).length} base movement values to ${outputPath}`);
}
